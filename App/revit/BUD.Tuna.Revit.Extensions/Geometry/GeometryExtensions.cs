...
public static class GeometryExtensions
{
    ...
    public static List<GeometryObject> ResolveGeometry(this Element element, Action<GeometryOptions> action)
    {
        ...
        return geometries.ResolveGeometry(options.GeometryType);
    }
    ...
    /// <summary>
    /// Get faces from <see cref="FaceArray"/>
    /// </summary>
        ...
        foreach (GeometryObject item in geometries)
        {
            switch (item)
            {
                case GeometryInstance geomInstance:
                    ...
                    objects.AddRange(subGeometryElement.ResolveGeometry(geometryType));
                    break;
                case GeometryElement geometryElement:
                    objects.AddRange(geometryElement.ResolveGeometry(geometryType));
                    break;
                default:
                    objects.Add(item);
                    break;
