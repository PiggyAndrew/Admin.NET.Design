...
using BUD.Tuna.Revit.Extensions.Utility;
using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.Linq;
using static BUD.Tuna.Revit.Extensions.Collection.ElementFilterFactory;
...
public static class CollectorExtension
{
    ...
    /// 但不是所有<see cref="Autodesk.Revit.DB.Element"/>派生的类都可以通过<see cref="ElementClassFilter"/>进行过滤
    ...
    /// <item><see cref="RoomTag"/></item>
    /// <item><see cref="Area"/></item>
    /// <item><see cref="AreaTag"/></item>
    /// <item><see cref="Space"/></item>
    /// <item><see cref="SpaceTag"/></item>
    /// </list>
    /// </summary>
    /// <remarks>如果是上面列表中的类型，将不会使用<see cref="ElementClassFilter"/>，而是选择慢速过滤器</remarks>
    /// <param name="document">要查询的文档</param>
    /// <param name="type">type of element</param>
    /// <returns>从文档中查询到的图元集合 <see cref="Autodesk.Revit.DB.FilteredElementCollector"/></returns>
    /// <exception cref="ArgumentException">类型必须继承于<see cref="Autodesk.Revit.DB.Element"/></exception>
    [DebuggerStepThrough]
    ...
    /// <returns></returns>
    public static IEnumerable<T> GetElements<T>(this Document document, ElementFilter filter) where T : Element
    {
        return document.GetElements(LogicalAnd(new ElementClassFilter(typeof(T)), filter)).Cast<T>();
    }
    ...
    /// 扩展包提供了常量类型<see cref="Extension.BuiltInCategories"/>可进行<c>Id</c>的调用
    ...
    {
        return document.GetElements<Family>(f => f.Name == familyName).FirstOrDefault();
    }
