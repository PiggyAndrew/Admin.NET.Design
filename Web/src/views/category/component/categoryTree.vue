	...
	<el-card class="box-card" shadow="hover" style="height: 100%" body-style="height:100%; overflow:auto">
			...
			<div class="card-header">
				...
				<div class="tree-h-flex">
					<div class="tree-h-left">
						<el-input :prefix-icon="Search" v-model="filterText" placeholder="分类名称" />
					</div>
		...
		<div style="margin-bottom: 45px" v-loading="state.loading">
			<el-tree
				...
				:default-checked-keys="state.ownCategoryData"
				highlight-current
				check-strictly
			>
...
import {SysCategoryApi, SysTenantApi} from '/@/api-services/api';
import { SysCategory } from '/@/api-services/models';
...
const state = reactive({
	...
	ownCategoryData: [] as Array<SysCategory>,
});
...
const initTreeData = async () => {
	state.loading = true;
	const res = await getAPI(SysCategoryApi).apiSysCategoryListGet(0, undefined, undefined, undefined, state.tenantId);
...
const setCheckedKeys = (data: any) => {
	...
	state.ownCategoryData = isArray ? data : [data];
