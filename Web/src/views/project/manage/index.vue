<template>
    <div class="sys-file-container">
        ...
        <el-container>
            <el-aside class="sidebar" width="260px"><!-- 左侧分类树 -->
                <div class="sidebar">
            ...
            <el-main class="full-table"><!-- 主内容区域 -->
                <el-card class="full-table" shadow="hover" style="margin-top: 5px">
                    <el-table :data="state.fileData" style="width: 100%" v-loading="state.loading" border>
                        ...
                        <el-table-column prop="url" label="预览" align="center">
                            <template #default="scope">
                                <el-image style="width: 60px; height: 60px" :src="getFileUrl(scope.row)" alt="无法预览"
                        ...
                        <el-table-column prop="fileType" label="文件类型" min-width="100" header-align="center"
                            show-overflow-tooltip />
                        <el-table-column prop="isPublic" label="是否公开" min-width="100" header-align="center"
                            show-overflow-tooltip>
                            <template #default="scope">
                        ...
                        <el-table-column label="操作" width="260" fixed="right" align="center" show-overflow-tooltip>
                            <template #default="scope">
                                <el-button-group>
                                    ...
                                    <el-button icon="ele-Download" size="small" type="primary"
                                        @click="downloadFile(scope.row)" v-auth="'sysFile:downloadFile'"></el-button>
                                    <el-button icon="ele-Delete" size="small" type="danger" @click="delFile(scope.row)"
                                        v-auth="'sysFile:delete'"></el-button>
                                    <el-button icon="ele-Edit" size="small" type="primary"
                                        @click="openEditSysFile(scope.row)" v-auth="'sysFile:update'"></el-button>
                                </el-button-group>
                            </template>
                        </el-table-column>
                    </el-table>
        ...
        <el-dialog v-model="state.dialogUploadVisible" :lock-scroll="false" draggable width="400px">
            ...
            <div>
                ...
                <el-upload ref="uploadRef" drag :auto-upload="false" :limit="1" :file-list="state.fileList" action
                    :on-change="handleChange" accept=".rfa">
        ...
        <el-drawer :title="state.fileName" v-model="state.dialogDocxVisible" size="50%" destroy-on-close>
            <vue-office-docx :src="state.docxUrl" style="height: 100vh" @rendered="renderedHandler"
                @error="errorHandler" />
        </el-drawer>
        <el-drawer :title="state.fileName" v-model="state.dialogXlsxVisible" size="50%" destroy-on-close>
            <vue-office-excel :src="state.excelUrl" style="height: 100vh" @rendered="renderedHandler"
                @error="errorHandler" />
        </el-drawer>
        <el-drawer :title="state.fileName" v-model="state.dialogPdfVisible" size="50%" destroy-on-close>
            <vue-office-pdf :src="state.pdfUrl" style="height: 100vh" @rendered="renderedHandler"
                @error="errorHandler" />
        </el-drawer>
